...
export default {
   ...
   * @returns
   */
  query(folderId: string, sort: SORT, order: ORDER, page: number, num: number, name?: string) {
    return get('/file', { folderId, sort, order, page, num })
  },
  /**
   * 根据ID获取文件
   * @param {string} id
   ...
   * @returns
   */
  upload(folder: string, file: any, name: string, size: number, modifyDate: number, chunk: number, chunks: number) {

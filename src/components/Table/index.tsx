          ...
          <div className={ styles.tr }>
            {
              this.props.select ? (
                <div className={ [styles.th, styles.select].join(' ') }>
                  <Checkbox onClick={ this.onClickCheckAllBtn } indeterminate={ !!checked.length && (checked.length < dataSource.length) } checked={ dataSource.length === checked.length } />
                </div>
              ) : null
            }
            {
              this.props.columns.map(col => {
                return (
                  <div key={ col.key } className={ styles.th } style={ this.columnWidth(col) }>
                    { col.label }
                  </div>
                )
              })
          ...
          <div className={ styles.tbody }>
            {
              this.props.dataSource.map(data => {
                return (
                  <div key={ data[this.props.dataIndex] } className={ styles.tr } onClick={ () => this.onClickColumn(data) }>
                      {
                        this.props.select ? (
